...
def run():
    ...
    for fname in files:
        ...
        # use last timestamp recorded into helper file
        if str(args.start).lower() == "last" or str(args.start).lower() == 'l':
            lastfile = args.csv_file + ".timestamp"
            try:
                fd = os.open(lastfile, os.O_RDONLY)
                timestamp = fd.readline()
        ...
        rrdtool_output[fname.get_alias()] = subprocess.check_output(call)
    ...
    lastfile = args.csv_file + ".timestamp"
    try:
        fd = os.open(lastfile, os.O_CREAT | os.O_WRONLY | os.O_EXCL)
        os.write(fd, "%s\n" % lasttimestamp)
        os.close(fd)
    except OSError:
...
def files_to_read(json_file, rrd_dir):
    """
    Return a list of file names to read and convert to CSV. The list is generated based on the parameters
    in settings file.
